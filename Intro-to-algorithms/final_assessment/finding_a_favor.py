...
def calculate_path(probabilities, v1, v2):

    node = v2
    path = [node]
    while node != v1:
        prob, node = probabilities[node]
        path.insert(0, node)
    return path


def check_graph(G, v1):

    n = len(G)
    m = 0
    for node in G:
        G[node] = {k: -math.log(v) for k, v in G[node].items()}
        m += len(G[node])
    if m*math.log(n) > n**2:
        probs = dijkstra_list(G, v1)
    else:
        probs = dijkstra_heap(G, v1)
    return probs
...
def maximize_probability_of_favor(G, v1, v2):
    ...
    if v1 == v2:
        return [v1], 1.0
    probs = check_graph(G, v1)
    if v2 not in probs:
        return [], 0.0
    return calculate_path(probs, v1, v2), math.exp(-probs[v2][0])
