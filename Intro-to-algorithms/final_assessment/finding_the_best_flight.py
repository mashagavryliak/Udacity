...
def convert_time(time_str):
    return datetime.combine(date.min, datetime.strptime(time_str, '%H:%M').time())
...
def create_graph(flights):
    ...
    cities = set([])
    flight_numbers = []
    for id, origin, destination, t1, t2, price in flights:
        # make_link(G, origin, destination, [price, (convert_time(t2) - convert_time(t1)).seconds])
        make_link(G, 'origin-' + origin, id, 0)
        make_link(G, id, 'destination-' + destination, price)
        cities.add(origin)
        cities.add(destination)
        flights_info[id] = {'start': convert_time(t1), 'end': convert_time(t2), 'origin': origin,
                            'destination': destination, 'price': price}
    for city in cities:
        # print 'city', city
        possible_destinations = [x for x in G['origin-'+city]]
        # print possible_destinations
        possible_origins = [x for x in flights_info if flights_info[x]['destination'] == city]
        for orig in possible_origins:
            for dest in possible_destinations:
                if flights_info[orig]['end'] < flights_info[dest]['start']:
                    make_link(G, orig, dest, flights_info[orig]['price'])

    return G, flights_info
...
def find_best_flights(flights, origin, destination):
    ...
    G, flight_info = create_graph(flights)
    origin = 'origin-'+origin
    destination = 'destination-' + destination
    path = dijkstra_path(G, origin).get(destination)
    if path is None:
        return None
    return path[1:-1]
